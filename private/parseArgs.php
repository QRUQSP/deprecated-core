...
// -------
//
function qruqsp_core_parseArgs(&$q, $station_id, $raw_args, $arg_info) {
    $args = array();

    //
    // Check if station_id is specified, and if so, load the station settings
    //
    if( $station_id > 0 ) {
    ...
    foreach($arg_info as $arg => $options) {
        ...
        if( isset($raw_args[$arg]) ) {
            ...
            if( $raw_args[$arg] == '' && isset($options['blank']) && $options['blank'] != 'yes' ) {
                return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.120', 'msg'=>"$msg", 'pmsg'=>"Argument: $arg missing"));
            }
            ...
            if( isset($options['type']) && $options['type'] == 'idlist' ) {
                ...
                if( $raw_args[$arg] == 'now' || $raw_args[$arg] == 'today' ) {
                    ...
                    if( $ts === FALSE ) { // Removed check for < 1 as negative timestamps work || $ts < 1 ) {
                        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.121', 'msg'=>"$invalid_msg", 'pmsg'=>"Argument: $arg invalid date format $ts"));
                ...
                if( $raw_args[$arg] == 'now' || $raw_args[$arg] == 'today' ) {
                    ...
                    $ts = strtotime($raw_args[$arg]);
                    if( $ts === FALSE || $ts < 1 ) {    
                        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.122', 'msg'=>"$invalid_msg", 'pmsg'=>"Argument: $arg invalid date format"));
            ...
            elseif( isset($options['type']) && $options['type'] == 'datetime' && $raw_args[$arg] != '' ) {
                if( $raw_args[$arg] == 'now' ) {
                    ...
                    $ts = strtotime($raw_args[$arg]);
                    if( $ts === FALSE ) {
                        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.123', 'msg'=>"$invalid_msg", 'pmsg'=>"Argument: $arg invalid datetime format"));
                    } else {
            ...
            elseif( isset($options['type']) && $options['type'] == 'datetimetoutc' && $raw_args[$arg] != '' ) {
                if( $raw_args[$arg] == 'now' ) {
                    ...
                    $ts = strtotime($raw_args[$arg]);
                    if( $ts === FALSE || $ts < 1 ) {
                        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.124', 'msg'=>"$invalid_msg", 'pmsg'=>"Argument: $arg invalid datetime format"));
                    } else {
            ...
            elseif( isset($options['type']) && $options['type'] == 'currency' && $raw_args[$arg] != '' ) {
                ...
                if( $args[$arg] != '' ) {
                    ...
                    if( $amt === FALSE ) {
                        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.125', 'msg'=>"$invalid_msg", 'pmsg'=>"Argument: $arg invalid currency format"));
                    }
                    $args[$arg] = $amt;
                }
            }
            ...
            if( isset($options['validlist']) && !in_array($args[$arg], $options['validlist']) ) {
                return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.126', 'msg'=>"$invalid_msg", 'pmsg'=>"Argument: $arg not an acceptable input"));
            }
        } 
        ...
        //
            ...
            if( $options['required'] == 'yes' ) {
                return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.core.127', 'msg'=>"$msg", 'pmsg'=>"Argument: $arg missing."));
            } 
